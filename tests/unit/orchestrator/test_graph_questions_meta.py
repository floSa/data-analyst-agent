...
def test_les_attributs_d_un_modele_designe_sont_rendus(registre: Registry):
    ...
    Les deux autres entrées sont là pour que l'outil ait un choix à faire.
    """
    champs = (
        "store_type",
        "commodity_group",
    ...
    llm = agent_systeme(
        ...
        {"modele": "maxizoo_sales"},
        "Pour **maxizoo_sales** (régression de `quantity`) il me faut : "
...
def test_les_modeles_sont_ceux_du_registre(registre: Registry):
    llm = agent_systeme(
        ...
        {},
        "Trois modèles : **iris** (cible `species`), **maxizoo_sales** (cible "
        "`quantity`) et **titanic** (cible `survived`).",
    )
...
def test_la_reponse_du_systeme_n_est_pas_repassee_a_la_synthese(mini_csv: Path, registre: Registry):
    ...
    llm = agent_systeme(
        ...
        "`mini`. Mes modèles : `iris`, `maxizoo_sales`, `titanic`.",
    )
    catalogue = Catalog(sources=[FileSource(name="mini", path=mini_csv)])

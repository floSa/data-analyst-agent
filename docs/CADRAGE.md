...
│   ├── agents/
│   │   ├── retrieval/            # catalog.py, sql.py, duckdb_source.py, agent.py
...
├── sources/                      # catalogue.yaml ; la base et son dictionnaire (non versionnés)
├── notebooks/                    # entraînement du modèle de prévision (jupytext .md + .ipynb)
├── scripts/                      # comptes, migration du workspace, chargement de la base, mesures, bancs
...
└── tests/
    ...
    ├── helpers/                  # ScriptedLLM, doublures, mini-base + oracle Maxizoo
    └── fixtures/                 # maxizoo_mini/ : la base réelle en miniature (425 Ko)
```
